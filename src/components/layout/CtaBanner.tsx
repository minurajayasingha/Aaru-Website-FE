...
export function CtaBanner({ imageSrc, mobileImageSrc, imageAlt, heading, paragraph }: CtaBannerContent) {
  return (
    <section className="relative flex h-[60vh] sm:h-[30vh] xl:h-[55vh] min-h-[420px] items-center justify-center overflow-hidden text-center">
      {mobileImageSrc ? (
        <>
          <Image src={mobileImageSrc} alt={imageAlt} fill className="object-cover md:hidden" />
          <Image src={imageSrc} alt={imageAlt} fill className="hidden object-cover md:block" />
        </>
      ) : (
