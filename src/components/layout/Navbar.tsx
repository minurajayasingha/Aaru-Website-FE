...
export function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="flex items-center justify-between gap-6 px-section-s py-4 lg:py-1 md:px-section-x">
        ...
        <ul className="hidden items-center gap-8 lg:flex xl:gap-10">
          {navLinks.map((link) => {
            ...
            const linkClasses = cn(
              "relative pb-[0.2rem] font-body text-body-nav font-light text-brand-forest-900 hover:text-brand-gold",
              active &&
                  ...
                  <ul className="absolute left-0 top-full min-w-[170px] rounded-card bg-white py-2 shadow-card ">
                    {link.children.map((child) => (
                      <li key={child.href}>
                        <Link
                          href={child.href}
                          className="block px-4 py-2 font-body text-body-nav font-light text-brand-forest-900 hover:bg-brand-forest-50 hover:text-brand-gold"
                        >
                          {child.label}
                        </Link>
                      </li>
              ...
              link.children ? (
                <li key={link.href} className="flex flex-col items-center gap-6">
                  <button
                    ...
                    className="m-0 inline-flex items-center gap-1.5 border-0 bg-transparent p-0"
                  >
                    <span className="font-body text-body-sm font-light text-brand-forest-900">{link.label}</span>
                    <svg
                      aria-hidden="true"
                      viewBox="0 0 10 6"
                    ...
                    <ul className="flex min-w-[200px] flex-col rounded-card bg-white py-2 shadow-card">
                      {link.children.map((child) => (
                        <li key={child.href}>
                          <Link
                            href={child.href}
                            className="block px-4 py-2 text-center font-body text-body-sm font-light text-brand-forest-900 hover:bg-brand-forest-50 hover:text-brand-gold"
                ...
                <li key={link.href}>
                  <Link
                    ...
                    className="font-body text-body-sm font-light text-brand-forest-900"
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {link.label}
                  </Link>
